...
static uint64 load_binary(pagetable_t target_page_table, const char *bin){
	...
	for (i = 0; i < elf->header.e_phnum; ++i) {
		if (elf->p_headers[i].p_type == PT_LOAD) {
             ...
             * 页表映射修改
             */
			for (int j=0;j<seg_map_sz/PGSIZE;++j){
				void *tmp=mm_kalloc();
				memset(tmp,0,PGSIZE);
				memcpy(tmp,bin+elf->p_headers[i].p_offset+j*PGSIZE,j==seg_map_sz/PGSIZE-1?seg_sz-j*PGSIZE:PGSIZE);
				pt_map_pages(target_page_table,p_vaddr+j*PGSIZE,(uint64)tmp,PGSIZE,PTE_R|PTE_X|PTE_U);
			}
		}
	}
	/* PC: the entry point */
...
pagetable_t proc_pagetable(thread_t *t){
	pagetable_t pagetable;
	pagetable=(pagetable_t)mm_kalloc();
	memset(pagetable,0,PGSIZE);
	pt_map_pages(pagetable,TRAMPOLINE,(uint64)trampoline,PGSIZE,PTE_R|PTE_X);
	pt_map_pages(pagetable,TRAPFRAME,(uint64)(t->trapframe),PGSIZE,PTE_R|PTE_W);
	t->trapframe->sp=PGSIZE*8;
	pt_map_pages(pagetable,t->trapframe->sp-PGSIZE,(uint64)mm_kalloc(),PGSIZE,PTE_R|PTE_W|PTE_U|PTE_X);
	return pagetable;
}
process_t *alloc_proc(const char* bin, thread_t **thr){
	...
	if(tpos==-1) return 0;
	process_t *p=&proc[ppos];
	thread_t *t=&thread[tpos];
	p->pid=allocpid();
...
void yield(){
	thread_t *t=mythread();
	running[cpuid()]=0;
	if(t->thread_state!=ZOMBIE) t->thread_state=RUNNABLE;
	sched_enqueue(t);
